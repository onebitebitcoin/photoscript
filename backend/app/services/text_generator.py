...
- 유튜브 영상처럼 친절하고 따뜻한 말투로 작성
- 초보자도 쉽게 이해할 수 있도록 어려운 용어는 풀어서 설명
- 입니다체(존댓말)로 작성
- 최소 5문장, 최대 10문장으로 작성
...
async def generate_block_text(
    ...
    if mode == TextGenerationMode.LINK:
        ...
        search_system_prompt = """당신은 유튜브 영상 스크립트 작성 전문가입니다.
웹 검색을 통해 최신 정보를 찾아, 친절하고 따뜻한 말투로 초보자도 쉽게 이해할 수 있는 영상 스크립트를 작성합니다.

규칙:
1. 유튜브 크리에이터처럼 친근하고 따뜻한 말투 사용
2. 어려운 용어는 쉽게 풀어서 설명
3. 입니다체(존댓말)로 작성
4. 이전/다음 블록과 자연스럽게 연결
5. 다른 설명 없이 스크립트 텍스트만 반환"""
    ...
    system_prompt = """당신은 유튜브 영상 스크립트 작성 전문가입니다.
친절하고 따뜻한 말투로, 초보자도 쉽게 이해할 수 있는 영상 스크립트를 작성합니다.
